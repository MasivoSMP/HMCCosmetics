...
public class HMCCPacketManager {
    ...
    public static void equipmentSlotUpdate(
        ...
        NMSHandlers.getHandler().getPacketBuilder().buildEntityEquipmentSlotUpdatePacket(entityId, items).sendPacket(sendTo);
    }
    ...
    public static void spawnInvisibleArmorstand(
        ...
        NMSHandlers.getHandler().getPacketSender().sendBundle(getInvisibleArmorStand(entityId, location, uuid), sendTo);
    }
    ...
    public static void spawnCloudAndHandleEffect(
        ...
        NMSHandlers.getHandler().getPacketSender().sendBundle(getCloudHandleEffect(entityId, location, uuid), sendTo);
    }
    ...
    public static void sendArmorStandHeadPose(
        ...
        NMSHandlers.getHandler()
                .getPacketBuilder()
                .buildArmorStandHeadPosePacket(entityId, (float) (pitchRadians * RADIANS_TO_DEGREES), 0.0F, 0.0F)
                .sendPacket(sendTo);
    }
    ...
    public static void sendCameraPacket(int entityId, List<Player> sendTo) {
        NMSHandlers.getHandler().getPacketBuilder().buildEntityCameraPacket(entityId).sendPacket(sendTo);
    }

    public static void gamemodeChangePacket(Player player, GameMode gameMode) {
        NMSHandlers.getHandler().getPacketBuilder().buildPlayerGamemodeChangePacket(gameMode).sendPacket(player);
    }
    ...
    public static void ridingMountPacket(
        ...
        sendRidingPacket(mountId, passengerId, sendTo);
    }
    ...
    public static void sendFakePlayerSpawnPacket(
        ...
        NMSHandlers.getHandler().getPacketBuilder().buildEntitySpawnPacket(entityId, uuid, EntityType.PLAYER, location).sendPacket(sendTo);
    }

    public static void sendScoreboardHideNamePacket(
            final Player player,
            final String name,
            final List<Player> sendTo
    ) {
        NMSPacketBuilder packetBuilder = NMSHandlers.getHandler().getPacketBuilder();
        List<PacketWrapper> packets = new ArrayList<>();
        packets.add(packetBuilder.buildPlayerScoreboardRemovePacket(player, name));
        packets.add(packetBuilder.buildPlayerScoreboardCreatePacket(player, name));
        packets.add(packetBuilder.buildPlayerScoreboardAddPlayersPacket(player, name));
        NMSHandlers.getHandler().getPacketSender().sendBundle(packets, sendTo);
    }
